...
interface Article {
  ...
  async function getArticles(): Promise<Article[]> {
    try {
      const querySnapshot = await getDocs(collection(db, "Featured World Economy"));
      const data: Article[] = [];
...
export default function FeaturedWorldEconomy() {
  ...
  const fetchComments = async (articleId: string) => {
    try {
      ...
      const commentsRef = collection(db, 'Featured World Economy');
