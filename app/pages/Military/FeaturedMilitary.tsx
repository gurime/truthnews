...
interface Article {
  ...
  async function getArticles(): Promise<Article[]> {
    try {
      const querySnapshot = await getDocs(collection(db, "Featured Military"));
      const data: Article[] = [];
...
export default function FeaturedMilitary() {
  ...
  const fetchComments = async (articleId: string) => {
    try {
      ...
      const commentsRef = collection(db, 'Featured Military');
