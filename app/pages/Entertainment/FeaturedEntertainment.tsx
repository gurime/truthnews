...
interface Article {
  ...
  async function getArticles(): Promise<Article[]> {
    try {
      const querySnapshot = await getDocs(collection(db, "Featured Entertainment"));
      const data: Article[] = [];
...
export default function FeaturedEntertainment() {
  ...
  const fetchComments = async (articleId: string) => {
    try {
      ...
      const commentsRef = collection(db, 'Featured Entertainment');
