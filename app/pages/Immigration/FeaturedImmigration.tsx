...
interface Article {
  ...
  async function getArticles(): Promise<Article[]> {
    try {
      const querySnapshot = await getDocs(collection(db, "Featured Immigration"));
      const data: Article[] = [];
...
export default function FeaturedImmigration() {
  ...
  const fetchComments = async (articleId: string) => {
    try {
      ...
      const commentsRef = collection(db, 'Featured Immigration');
