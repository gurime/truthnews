...
interface Article {
  ...
  async function getArticles(): Promise<Article[]> {
    try {
      const querySnapshot = await getDocs(collection(db, "Featured Video Games"));
      const data: Article[] = [];
...
export default function FeaturedVideoGames() {
  ...
  const fetchComments = async (articleId: string) => {
    try {
      ...
      const commentsRef = collection(db, 'Featured Video Games');
